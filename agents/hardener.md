...
**See [`SECURITY_RULES.md`](../SECURITY_RULES.md) for the complete, authoritative list of all security requirements.** This is the single source of truth for all security rules across all phases.

Key areas to audit:
- **Secrets** — no API keys, credentials, or tokens in code/public env vars
- **Database** — Row Level Security on all tables
- **Authentication** — middleware checks before handlers, ownership verification, proper status codes
- **Input & Output** — parameterized queries, sanitization, server-side validation
...
- **Dependencies** — version pinning, lock files committed
